...
def create_user(user: schemas.User, response: Response, db: Session = Depends(get_db)):
    check_user = db.query(models.User).filter(or_(models.User.registerNumber == user.registerNumber, models.User.personalEmail == user.personalEmail, models.User.officialEmail == user.officialEmail)).first()
    if not check_user:
        user.id = uuid4().hex
